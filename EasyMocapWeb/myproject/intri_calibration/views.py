...
def get_intri_yml(request):
    ...
    if request.method == 'POST':
        # Upload videos for intinsic calibration
        ...
        if form.is_valid():
            ...
            upload_dir = os.path.join(root_dir, 'intri_data', 'videos')
        ...
        calib_script_path = os.path.join(settings.BASE_DIR, 'EasyMocap', 'apps', 'calibration', 'calib_intri.py')

        # The required path argument in the expected format
        data_directory = 'EasyMocap/intri_data'

        # Check if the extract script file exists
        ...
        # Check if the intri_calib script file exists
        if not os.path.isfile(calib_script_path):
            return HttpResponse("Calib script not found.", status=404)
        
        # Run the intri_calib script using subprocess
        try:
            calib_command = f'python3 {calib_script_path} {data_directory}'
        ...
        url = reverse('get_extri_yml')
        success_html = f"""
        <html>
            <body>
                <h1>Success!</h1>
                <p>Your operation was successful.</p>
                <p>Click here to go to <a href="{url}">extrinsic calibration</a>.</p>
            </body>
        </html>
        """
    ...
    return render(request, 'intri_calibration/get_intri_yml.html', {
